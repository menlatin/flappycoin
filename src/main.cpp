...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, int nHeight)
{
    ...
    if(nHeight >= CUTOFF_HEIGHT + 100000 && nHeight < CUTOFF_HEIGHT + 150000)
        nRewardCoinYear = nRewardCoinYear * 0.8;
    if(nHeight >= CUTOFF_HEIGHT + 150000 && nHeight < CUTOFF_HEIGHT + 200000)
        nRewardCoinYear = nRewardCoinYear * 0.7;
    if(nHeight >= CUTOFF_HEIGHT + 200000 && nHeight < CUTOFF_HEIGHT + 250000)
        nRewardCoinYear = nRewardCoinYear * 0.6;
    if(nHeight >= CUTOFF_HEIGHT + 250000 && nHeight < CUTOFF_HEIGHT + 300000)
        nRewardCoinYear = nRewardCoinYear * 0.5;
    if(nHeight >= CUTOFF_HEIGHT + 300000 && nHeight < CUTOFF_HEIGHT + 350000)
        nRewardCoinYear = nRewardCoinYear * 0.4;
    if(nHeight >= CUTOFF_HEIGHT + 350000 && nHeight < CUTOFF_HEIGHT + 400000)
        nRewardCoinYear = nRewardCoinYear * 0.3;
    if(nHeight >= CUTOFF_HEIGHT + 400000 && nHeight < CUTOFF_HEIGHT + 450000)
        nRewardCoinYear = nRewardCoinYear * 0.2;
    if(nHeight >= CUTOFF_HEIGHT + 450000)
        nRewardCoinYear = nRewardCoinYear * 0.1;
...
void CBlockHeader::UpdateTime(const CBlockIndex* pindexPrev)
{
    ...
    if (fTestNet)
        nBits = GetNextWorkRequired(pindexPrev, this, pindexPrev->);
    */
}
...
bool CBlock::AddToBlockIndex(CValidationState &state, const CDiskBlockPos &pos,  const uint256& hashProofOfStake)
{
    // Check for duplicate
    ...
    if (!CheckStakeModifierCheckpoints(pindexNew->nHeight,pindexNew))
        return error("AddToBlockIndex() : Rejected by stake modifier checkpoint height=%d, checksum=%08x, correct checksum=%08x,"
                     " nflags = %i, modifier=0x%016"PRI64x " hashproofofstake = %s",
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    try {
        ...
        if (!AddToBlockIndex(state, blockPos, hashProofOfStake))
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        try {
            ...
            if (!block.AddToBlockIndex(state, blockPos, 0))
